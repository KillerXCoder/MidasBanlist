...
$sql = "SELECT * FROM litebans_mutes WHERE active=1 ORDER BY id DESC";
...
$sql = "SELECT * FROM litebans_mutes WHERE active=1 ORDER BY id DESC LIMIT ".$offset.",". $pocet_na_stranu;
$result = $conn->query($sql);
...
if (isset($_GET['nick'])) {
	...
	$sql = "SELECT * FROM litebans_mutes WHERE active=1 AND uuid=\"" . $uuid . "\" ORDER BY id DESC ";
	$result = $conn->query($sql);
	...
	{
	echo "<br><h3 style='text-align:center; color:red;'>Hráč s nickom " . $nickname . " nie je umlčaný!</h3>";
	}
	$conn->close();

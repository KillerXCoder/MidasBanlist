...
$sql = "SELECT * FROM litebans_warnings WHERE active=1 ORDER BY id DESC";
...
$sql = "SELECT * FROM litebans_warnings WHERE active=1 ORDER BY id DESC LIMIT ".$offset.",". $pocet_na_stranu;
$result = $conn->query($sql);
...
echo "<h1 style='text-align:center;'>Upozornenia</h1><br>";
...
echo '<a href="https://midascraft.sk/umlcania-2/" class="pravidla">UMLČANIA</a>&nbsp&nbsp&nbsp&nbsp';
echo '<a href="https://midascraft.sk/vykopnutia/" class="pravidla">VYKOPNUTIA</a>';
echo '</div>';
echo "<br><p style='text-align:center;'>Celkový počet aktívnych upozornení: ";
echo "<h2 style='text-align:center; font-weight:bold;'><i class='fas fa-exclamation-circle'></i> ". $celkovo_bany . "</h2></p>";
echo '<br>
...
if (isset($_GET['nick'])) {
	...
	$sql = "SELECT * FROM litebans_warnings WHERE active=1 AND uuid=\"" . $uuid . "\" ORDER BY id DESC ";
	$result = $conn->query($sql);
	...
	if ($result->num_rows > 0) {
		...
		while($row = $result->fetch_assoc()) {
			...
			if($row['until'] == -1){
				...
				$koniec = date('d.m.Y H:i',strtotime("+1 hour",$row['until']/1000));
			}
			
			echo '<tr><td align="center"><img style="margin-right: 7px; border-radius: 5px" src="https://cravatar.eu/head/'.$meno2.'" width="24px" height="24px"> <br>'.$meno2 .'</td><td align="center"><img style="margin-right: 7px; border-radius: 5px" src="https://cravatar.eu/head/'.$row['banned_by_name'].'" width="24px" height="24px"> <br>'. $row['banned_by_name'] .'</td><td align="center">'. $row['reason'] .'</td><td align="center">'. gmdate("d.m.Y H:i",strtotime("+1 hour",$row['time']/1000)) .'</td><td align="center">'. $koniec .'</td></tr>';
		}
		echo '</tbody></table>';
		echo '</div>';
	} 
	...
	{
	echo "<br><h3 style='text-align:center; color:red;'>Hráč s nickom " . $nickname . " nemá aktívne žiadne upozornenie!</h3>";
	}
	$conn->close();

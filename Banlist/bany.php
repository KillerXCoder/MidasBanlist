...
$sql = "SELECT * FROM litebans_bans WHERE active=1 ORDER BY id DESC";
...
$sql = "SELECT * FROM litebans_bans WHERE active=1 ORDER BY id DESC LIMIT ".$offset.",". $pocet_na_stranu;
$result = $conn->query($sql);
...
echo "<h1 style='text-align:center;'>Banlist</h1><br>";
...
echo '</div>';
echo "<br><p style='text-align:center;'>Celkový počet aktívnych banov: ";
echo "<h2 style='text-align:center; font-weight:bold;'><i class='fas fa-gavel'></i> ". $celkovo_bany . "</h2></p>";
echo '<br>
...
if (isset($_GET['nick'])) {
	...
	$sql = "SELECT * FROM litebans_bans WHERE active=1 AND uuid=\"" . $uuid . "\" ORDER BY id DESC ";
	$result = $conn->query($sql);
	...
	if ($result->num_rows > 0) {
		...
		while($row = $result->fetch_assoc()) {
			...
			echo '<tr><td align="center"><img style="margin-right: 7px; border-radius: 5px" src="https://cravatar.eu/head/'.$meno2.'" width="24px" height="24px"> <br>'.$meno2 .'</td><td align="center"><img style="margin-right: 7px; border-radius: 5px" src="https://cravatar.eu/head/'.$row['banned_by_name'].'" width="24px" height="24px"> <br>'. $row['banned_by_name'] .'</td><td align="center">'. $row['reason'] .'</td><td align="center">'. date("d.m.Y H:i",strtotime("+1 hour",$row['time']/1000)) .'</td><td align="center">'. $koniec .'</td></tr>';
		}
		echo '</tbody></table>';
		echo '</div>';
	} 
